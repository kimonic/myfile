...
//        paintSel=initPaint(Color.parseColor("#FFFFFF"));
    ...
    protected void onDraw(Canvas canvas) {
        if (viewPager!=null){
            dotNum=viewPager.getChildCount();
            int firstX=getWidth()-dotRadius*2*dotNum-dotDistance*(dotNum-1)/2;
            int circleY=getHeight()/2;
            for (int i = 0; i < dotNum; i++) {
                canvas.drawCircle(firstX+i*(dotRadius+dotDistance),circleY,dotRadius,paintSel);
            }
